...
# Script to run ARM64 Docker container for Cochl Edge SDK
# Usage: ./script/run-arm64.sh [command]
...
CONTAINER_NAME="${CONTAINER_NAME:-cochl-edge-sdk-arm64}"
PLATFORM="linux/arm64"

echo "=================================================="
echo "Cochl Edge SDK - ARM64 Docker Runner"
echo "=================================================="
...
if ! docker image inspect "$IMAGE_NAME:$IMAGE_TAG" &>/dev/null; then
    ...
    echo "  cd docker"
    echo "  docker build --platform linux/arm64 -f Dockerfile.linux-arm64 -t $IMAGE_NAME:$IMAGE_TAG .."
    exit 1
fi
